...
public class MapperUtil {
	...
	public static <T> List<T> mapList(Class<T> clazz, ResultSet resultSet) {
		if (clazz == null || resultSet == null) {
			return null;
		}

		List<Map<String, Object>> datas = JDBCUtil.resultSetToListHashMap(resultSet);
	...
	public static <T> T map(Class<T> clazz, Map<String, Object> data) {
		...
		try {
			...
			// đồng bộ key về chữ thường và loại bỏ dấu gạch dưới: column1 = COLUMN1 =
			// Column_1 = colum_n1
			for (String key : data.keySet()) {
				String newKey = StringUtil.toCamel(key, false).toLowerCase();
			...
			List<Field> fields = ObjectUtil.getObjectField(object, true);
